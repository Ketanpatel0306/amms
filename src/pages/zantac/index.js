...
import { fetchZantacData } from "src/store/zantac";
  ...
  return (
    <>
      <MenuItem onClick={handleRowOptionsClose} sx={styles.menuItem()}>
        <Link href={{ pathname: "/zantac/view", query: { id: id } }}>
          <MenuItemLink>
...
const Zantac = () => {
  ...
  // ** Hooks
  const dispatch = useDispatch();
  const store = useSelector((state) => state.zantac);

  useEffect(() => {
    dispatch(fetchZantacData());
  }, [dispatch, value]);
