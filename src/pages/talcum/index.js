...
import { fetchTalcumData } from "src/store/talcum";
  ...
  return (
    <>
      <MenuItem onClick={handleRowOptionsClose} sx={styles.menuItem()}>
        <Link href={{ pathname: "/talcum/view", query: { id: id } }}>
          <MenuItemLink>
...
const Talcum = () => {
  ...
  // ** Hooks
  const dispatch = useDispatch();
  const store = useSelector((state) => state.talcum);

  useEffect(() => {
    dispatch(fetchTalcumData());
  }, [dispatch, value]);
