...
import { fetchHerniameshData } from "src/store/herniamesh";
  ...
  return (
    <>
      <MenuItem onClick={handleRowOptionsClose} sx={styles.menuItem()}>
        <Link href={{ pathname: "/herniamesh/view", query: { id: id } }}>
          <MenuItemLink>
...
const Herniamesh = () => {
  ...
  // ** Hooks
  const dispatch = useDispatch();
  const store = useSelector((state) => state.herniamesh);

  useEffect(() => {
    dispatch(fetchHerniameshData());
  }, [dispatch, value]);
