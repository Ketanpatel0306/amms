...
import { fetchRoundupData } from "src/store/roundup";
  ...
  return (
    <>
      <MenuItem onClick={handleRowOptionsClose} sx={styles.menuItem()}>
        <Link href={{ pathname: "/roundup/view", query: { id: id } }}>
          <MenuItemLink>
...
const Roundup = () => {
  ...
  // ** Hooks
  const dispatch = useDispatch();
  const store = useSelector((state) => state.roundup);

  useEffect(() => {
    dispatch(fetchRoundupData());
  }, [dispatch, value]);
