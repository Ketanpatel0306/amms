...
import { fetchCampLejuneData } from "src/store/camp-lejune";
  ...
  return (
    <>
      <MenuItem onClick={handleRowOptionsClose} sx={styles.menuItem()}>
        <Link href={{ pathname: "/camp-lejune/view", query: { id: id } }}>
          <MenuItemLink>
...
const CampLejune = () => {
  ...
  // ** Hooks
  const dispatch = useDispatch();
  const store = useSelector((state) => state.campLejune);

  useEffect(() => {
    dispatch(fetchCampLejuneData());
  }, [dispatch, value]);
