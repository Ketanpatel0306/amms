...
import { fetchParaquatData } from "src/store/paraquat";
  ...
  return (
    <>
      <MenuItem onClick={handleRowOptionsClose} sx={styles.menuItem()}>
        <Link href={{ pathname: "/paraquat/view", query: { id: id } }}>
          <MenuItemLink>
...
const Paraquat = () => {
  ...
  // ** Hooks
  const dispatch = useDispatch();
  const store = useSelector((state) => state.paraquat);

  useEffect(() => {
    dispatch(fetchParaquatData());
  }, [dispatch, value]);
