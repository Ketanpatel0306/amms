...
import CanViewNavLink from 'src/layouts/components/acl/can- view-navlink'
...
const VerticalNavLink = ({
  ...
  const isNavLinkActive = () => {
    if (router.pathname === item.path || router.pathname.includes(item.path) || handleURLQueries(router, item.path)) {
      return true
    } else {
      return false
    }
